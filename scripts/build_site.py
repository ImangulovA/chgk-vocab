...
<div class="legend">Бледный ряд = корпус меньше 25&nbsp;000 слов. Клик по строке раскрывает разбор.</div>
<div id="list"></div>
"""
    script = """
<script>
...
const qEl=q0('q'),sortEl=q0('sort'),paleEl=q0('pale');function q0(i){return document.getElementById(i);}
...
function render(){
  ...
  q0('list').innerHTML=rows.map((r,i)=>{const wd=Math.max(3,r[key]/max*100);
    ...
    return `<div class="row ${r.reliable?'':'pale'}" data-i="${i}">
      ...
      </div></div>`;}).join('')||'<p class="muted" style="padding:20px 0">GAME OVER — ничего не найдено.</p>';
}
q0('list').addEventListener('click',e=>{
  if(e.target.closest('a'))return; const row=e.target.closest('.row'); if(!row)return;
  const nx=row.nextElementSibling;
  if(nx&&nx.classList.contains('drow')){nx.remove();return;}
  document.querySelectorAll('.drow').forEach(d=>d.remove());
...
<p class="lead">Для каждого автора строим вектор его лексики (tf-idf по нарицательным словам) и ищем троих
с самым похожим профилем по косинусной близости. Это «пишут в одной манере», а не «про одно и то же».</p>
