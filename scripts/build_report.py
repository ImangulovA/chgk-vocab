...
# снимаем знаки ударения (U+0301/U+0300) до токенизации: «Дека́рт» иначе рвётся
_STRESS_TBL = dict.fromkeys([0x0300, 0x0301, 0x0341, 0x0342, 0x00B4, 0x02CA], None)
def destress(s):
    return s.translate(_STRESS_TBL)

# детект бел/укр по буквам і ї є ґ ў (в русском их нет; гомограф «гэта»-обувь не пострадает)
_OTHER = set("іїєґўІЇЄҐЎ")
_CYR_ALL = re.compile("[а-яёіїєґўА-ЯЁІЇЄҐЎ]")
def nonrus(q):
    t = destress((q.get("text", "") or "") + " " + (q.get("comment", "") or ""))
